...
class _DefaultApi:
    ...
    def _build_for_route_generate_image_image_get(self, prompt: str, api_token: str) -> Awaitable[m.Any]:
        query_params = {"prompt": str(prompt), "api_token": str(api_token)}
    ...
    def _build_for_route_generate_image_verified_image_verified_get(
        self, prompt: str, api_token: str, max_attempts: int = None
    ) -> Awaitable[m.Any]:
        query_params = {
            "prompt": str(prompt),
...
class AsyncDefaultApi(_DefaultApi):
    async def route_generate_image_image_get(self, prompt: str, api_token: str) -> m.Any:
        return await self._build_for_route_generate_image_image_get(prompt=prompt, api_token=api_token)
    ...
    async def route_generate_image_verified_image_verified_get(
        ...
        return await self._build_for_route_generate_image_verified_image_verified_get(
            prompt=prompt, api_token=api_token, max_attempts=max_attempts
        )
...
class SyncDefaultApi(_DefaultApi):
    def route_generate_image_image_get(self, prompt: str, api_token: str) -> m.Any:
        coroutine = self._build_for_route_generate_image_image_get(prompt=prompt, api_token=api_token)
